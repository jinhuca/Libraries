...
// Function returns true if initialization was successful, otherwise
// it returns false.
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR pCmdLine, int nShowCmd) {
  ...
  // Once our application has been created and initialized we enter the message loop.
  // We stay in the message loop until a WM_QUIT message is received, indicating
...
bool InitWindowsApp(HINSTANCE instanceHandle, int show) {
  ...
  // of zero. A window handle is a way to refer to the window, which is
  // internally managed by Windows. Many of the Win32 API functions that
  // operate on Windows requires an HWND so that they know what window 
  // to act on.
  ...
  // Therefore, the final step is to show and update the window we just created, 
  // which can be done with the following two functions called. Observe that 
  // we pass the handle to the window we want to show and update so that 
  // these functions know which window to show and update.
...
int Run() {
  ...
  // return 0 (false) when a WM_QUIT message is received, which effectively exits
  // the loop. The function returns -1 if there is an error. Also, note that
  // GetMessage puts the application thread to sleep until there is a message.
  BOOL bRet = 1;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  // Handle some specific message. Note that if we handle a message, we should return 0.
  switch (msg) {
  ...
  case WM_KEYDOWN:
    ...
    // In the case of a destroy message, then send a quit message, which will terminate
    // the message loop.
  case WM_DESTROY:
    PostQuitMessage(0);
    return 0;
  ...
  // Forward any other message we did not handle above to the default window procedure.
