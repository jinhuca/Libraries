/*
  ...
  . For any property you have defined in your QML components,
    QML will set up a signal named <propertyName>Changed, and you can handle it through
    the on<PropertyName>Changed handler, with the property name starting with captial

  . These signals are generated for properties, not signals
    For example, for MouseArea, we don't have an onClickedChanged signal, because
    clicked is a signal, not a property.
